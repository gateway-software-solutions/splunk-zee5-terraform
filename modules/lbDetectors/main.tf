...
variable "critical_detector" {default = "true"}
variable "severity" { default = "Warning"}
variable "delay_in_sec" {default=180}
...
variable "display_unit_prefix" {default = null}
variable "display_unit_suffix" {default = null}
...
resource "signalfx_detector" "detector_zee_custom_critical" {
  ...
  viz_options {
    ...
    value_prefix = var.display_unit_prefix
    value_suffix = var.display_unit_suffix
  }
}

resource "signalfx_detector" "detector_zee_custom_critical_and_warning" {
  count       = var.critical_detector ? 0 : 1
  name        = "${var.alert_name}-${var.application_name}"
  description = "Detector created using terraform for service ${var.application_name}"
  max_delay   = var.delay_in_sec
  timezone    = "Asia/Kolkata"
